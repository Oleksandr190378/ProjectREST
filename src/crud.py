...
async def get_contact(db: Session, contact_id: int):
    result = db.execute(select(models.Contact).where(models.Contact.id == contact_id))
    return result.scalars().first()


async def get_contact_by_email(db: Session, email: str):
    result = db.execute(select(models.Contact).where(models.Contact.email == email))
    return result.scalars().first()


async def get_contact_by_phone(db: Session, phone_number: str):
    result = db.execute(select(models.Contact).where(models.Contact.phone_number == phone_number))
    return result.scalars().first()
